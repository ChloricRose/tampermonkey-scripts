...
(function () {
  ...
  let observer = null;
  let detailsVisible = false; // State variable to track visibility of details columns
  ...
  const processSlots = debounce(() => {
    ...
    const lateSummary = { ...earlySummary };

    document.querySelectorAll('.appointment_slot').forEach(slot => {
      const slotClass = slot.className.split(' ')[1];
      const iconClass = slot.querySelector('i').className;
      const units = parseInt(slot.querySelector('.units').textContent, 10);
      const isVendor = iconLegend[iconClass] === 'Vendor';
      const isTSI = iconLegend[iconClass] === 'TSI';
      const isParcel = iconLegend[iconClass] === 'Parcel';
      const isOcean = iconLegend[iconClass] === 'Ocean';

      if (isVendor) {
        earlySummary.vendorCount++;
        earlySummary.vendorUnits += units;
        updateSlotCount(earlySummary, slotClass, 'Vendor');
      }
      if (isTSI) {
        earlySummary.tsiCount++;
        earlySummary.tsiUnits += units;
        updateSlotCount(earlySummary, slotClass, 'TSI');
      }
      if (isOcean) {
        earlySummary.oceanCount++;
        earlySummary.oceanUnits += units;
        updateSlotCount(earlySummary, slotClass, 'Ocean');
      }
      if (isParcel) {
        earlySummary.parcelCount++;
        earlySummary.parcelUnits += units;
        updateSlotCount(earlySummary, slotClass, 'Parcel');
      }

      if (isVendor || isTSI || isOcean || isParcel) {
        earlySummary.totalSlots++;
        earlySummary.totalUnits += units;
      }
    });
